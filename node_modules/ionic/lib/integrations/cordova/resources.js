 ...
 */
async function createImgDestinationDirectories(imgResources) {
 ...
 */
async function uploadSourceImage(env, srcImage) {
    let { req } = await http_2.createRequest('POST', UPLOAD_URL, env.config.getHTTPConfig());
    req = req
        .type('form')
 ...
 */
async function transformResourceImage(env, resource) {
    const { req } = await http_2.createRequest('POST', TRANSFORM_URL, env.config.getHTTPConfig());
            ...
            if (res.status !== 200) {
                ...
                buf.on('finish', () => {
                    ...
                    try {
                        ...
                        serverMsg = http_1.formatResponseError(req, res.status, serverMsg);
                    }
                    result.error = new Error(`Error while generating ${color_1.strong(resource.name)}: ` +
                        `${generalErrorMsg}\n\n` +
                        `Server: "${serverMsg}"`);
                    resolve(result);
                });
 ...
 */
async function addResourcesToConfigXml(conf, platformList, resourceJson) {
